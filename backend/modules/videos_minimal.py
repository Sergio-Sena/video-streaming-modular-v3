...
def handler(event, context):
    ...
    try:
        ...
        if event['httpMethod'] == 'GET':
            ...
            if show_hierarchy:
                return get_hierarchy_view(s3_client, current_path, headers)
            else:
                return get_simple_view(s3_client, headers)
...
def get_hierarchy_view(s3_client, current_path, headers):
...
def get_simple_view(s3_client, headers):
